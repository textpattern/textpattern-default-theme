...
This project is a replacement for the default theme that comes as standard with the Textpattern CMS (as of v4.5). It is intended as a starting point for new users learning the Textpattern CMS for the first time or existing users that want to adapt their current code for modern standards, and is not intended as a finished production theme (though you could use it as such if you want to).
...
The @<txp:author />@ tag currently can't be used in an article list context. "This should be fixed by an upcoming Textpattern patch":http://forum.textpattern.com/viewtopic.php?pid=258863#p258863. For now tag error message will display when filtering articles by author. 
